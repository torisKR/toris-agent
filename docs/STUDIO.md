...
4. After that turn is accepted, the server consumes the tray (`DesignStore.clearTray` / `saveTray({ items: [] })` / `presentTray`) so the chrome chip goes quiet at 0 without a separate Clear click. Capture files stay on disk so you can re-queue them. A rejected or failed turn (including missing Origin/session token) does not clear. A turn that does not attach tray items leaves the tray alone. GET never clears.
