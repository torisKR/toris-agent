...
3. Optionally write a short per-element note, pick more elements, then send **one** instruction with the whole tray. That payload is persisted under `~/.toris/studio/design/` (`des_*.json`, optional `des_*.png`, and `tray.json`) and attached to a single `POST /api/agent/turn` (`tray: true`).
4. After that turn is accepted, the server consumes the items that turn attached (`DesignStore.clearTray` when none remain) so the chrome chip goes quiet at 0 without a separate Clear click. Capture files stay on disk so you can re-queue them. Annotations added while the turn is running stay in the tray. A rejected or failed turn (including missing Origin/session token) does not clear. A turn that does not attach tray items leaves the tray alone. GET never clears.
5. When the tray has items, **Clear tray** asks for confirm, then empties `tray.json` (`POST /api/design/tray/clear` → `DesignStore.clearTray`). Capture files stay on disk. Missing Origin/session token is HTTP 403 and writes nothing. GET never clears. After success the tray UI refreshes and the chrome chip goes quiet at 0.
6. The left rail **Past captures** section lists captures still on disk that are not currently in the tray (`GET /api/design/captures`). **트레이에 넣기** re-queues one via `POST /api/design/tray/items` (`DesignStore.addToTray`). No new chrome chip. Hidden when every capture is already on the tray (or none exist).
