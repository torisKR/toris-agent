...
test('product shell contains queue, editor, media review, quality, and guarded publish regions', async () => {
  ...
  for (const id of ['review-queue', 'content-workspace', 'post-form', 'video-import', 'media-preview', 'quality-panel', 'evidence-receipt', 'publish-dialog', 'live-status']) {
...
test('product assets use only local API paths and define all responsive shells', async () => {
  ...
  for (const path of ['/api/session', '/api/contents', '/upload', '/media']) assert.match(js, new RegExp(path.replaceAll('/', '\\/')));
