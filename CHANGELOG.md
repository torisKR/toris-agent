...
- **Studio consume design tray on turn** — a successful `POST /api/agent/turn` that attaches the Design Mode tray (`tray: true`) empties `tray.json` (`DesignStore.clearTray`) after the request is accepted, so the existing design chip goes quiet at 0 without a separate Clear click. Capture files stay on disk. A rejected or failed turn (including missing Origin/session token) does not clear. A turn that omits tray leaves the tray alone. GET never clears. Manual **Clear tray** on `/design` is unchanged. See `docs/STUDIO.md`.
