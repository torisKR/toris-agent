...
- **Studio create agent profile** — `/agent` has a small **Create** form (`id`, `title`, `category`, `writes`, `summary`, optional `system`). Submit writes one `<cwd>/.toris/agents/<id>.json` through `writeAgentProfile` / `parseAgentProfile` — the same JSON the CLI already loads. Invalid id is HTTP 400; duplicate project file is HTTP 409; missing Origin/session token is HTTP 403. None of those write. After success the catalogue refreshes and the new id is selectable. GET never writes. Does not write `~/.toris/agents/`. No edit/delete. See `docs/STUDIO.md` and `docs/AGENTS.md`.
