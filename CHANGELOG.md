...
- **Studio consume design tray on turn** — a successful `POST /api/agent/turn` that attaches the Design Mode tray (`tray: true`) removes the items that turn attached (`DesignStore.clearTray` when none remain) after the request is accepted, so the existing design chip goes quiet at 0 without a separate Clear click. Capture files stay on disk. Annotations added while the turn is running stay. A rejected or failed turn (including missing Origin/session token) does not clear. A turn that omits tray leaves the tray alone. GET never clears. Manual **Clear tray** on `/design` is unchanged. See `docs/STUDIO.md`.
- **Studio clear design tray** — when `/design` has annotation tray items, **Clear tray** asks for confirm, then empties `tray.json` (`POST /api/design/tray/clear` → `DesignStore.clearTray`). Capture files stay on disk. Missing Origin/session token is HTTP 403 and writes nothing. GET never clears. After success the tray UI refreshes and the existing design chip goes quiet at 0. See `docs/STUDIO.md`.
- **Studio delete agent profile** — when a project-local profile is selected on `/agent`, **Delete** asks for confirm, then removes that one `<cwd>/.toris/agents/<id>.json` (`DELETE /api/agents/:id` → `deleteAgentProfile`). Unknown / home / builtin ids are HTTP 404; missing Origin/session token is HTTP 403. None of those delete. After success the catalogue refreshes and the selection clears if that id was selected. GET never deletes. Does not touch `~/.toris/agents/`. See `docs/STUDIO.md` and `docs/AGENTS.md`.
- **Studio edit agent profile** — when a project-local profile is selected on `/agent`, an **Edit** form prefills `title`, `category`, `writes`, `summary`, and optional `system` (`id` is immutable). **Save** (`PUT /api/agents/:id` → `updateAgentProfile` / `parseAgentProfile`) replaces that one `<cwd>/.toris/agents/<id>.json`. Unknown / home / builtin ids are HTTP 404; invalid body is HTTP 400; missing Origin/session token is HTTP 403. None of those write. After success the catalogue refreshes. GET never writes. Does not write `~/.toris/agents/`. No id rename. See `docs/STUDIO.md` and `docs/AGENTS.md`.
- **Studio design tray chip** — shared chrome shows a quiet read-only chip for the Design Mode annotation tray from `GET /api/design/tray` (`DesignStore.getTray` / `presentTray`). Click opens `/design`. Hidden when the count is 0. GET only; no tray write, clear, or turn. See `docs/STUDIO.md`.
- **Studio create agent profile** — `/agent` has a small **Create** form (`id`, `title`, `category`, `writes`, `summary`, optional `system`). Submit writes one `<cwd>/.toris/agents/<id>.json` through `writeAgentProfile` / `parseAgentProfile` — the same JSON the CLI already loads. Invalid id is HTTP 400; duplicate project file is HTTP 409; missing Origin/session token is HTTP 403. None of those write. After success the catalogue refreshes and the new id is selectable. GET never writes. Does not write `~/.toris/agents/`. See `docs/STUDIO.md` and `docs/AGENTS.md`.
- **Studio custom agent profiles** — `/agent` lists the same catalogue as `toris agents`, including `.toris/agents/*.json` (project) and `~/.toris/agents/*.json` (home). Custom rows are labeled `project` / `home`. Selecting one sends that id on the existing `POST /api/agent/turn`. `GET /api/agents` is read-only and skips a broken JSON file instead of 500. See `docs/STUDIO.md` and `docs/AGENTS.md`.
...
- **Studio `/brief` daily budget** — the secretary page shows today's spend, the configured `maxDailyCostUsd` (quiet when unset), and remaining. A small form **Save**s or **Clear**s that ceiling through `config.maxDailyCostUsd` (`POST /api/brief/budget` → `setDailyBudget`), the same field CLI `checkBudget` already uses. Invalid numbers are HTTP 400; missing Origin/session token is HTTP 403; neither writes. GET never writes. See `docs/BRIEF.md` and `docs/STUDIO.md`.
- **Studio `/knowledge` unlink edge** — each listed DAG edge has a quiet **Unlink** that confirms, then drops exactly one matching `{ from, to, kind }` through `KnowledgeStore.unlink`. Nodes stay on disk. Unknown edge is HTTP 404; missing Origin/session token is HTTP 403. Neither writes. After success the panel reloads the existing DAG GET. GET does not write. See `docs/KNOWLEDGE.md` and `docs/STUDIO.md`.
