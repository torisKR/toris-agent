...
- **Studio `/daemon`** — loopback page for worker status, local schedules, and recent jobs from the same store as `toris daemon status|schedule`. Enable/disable/remove/add use Origin + session token. Does not start or stop the worker. See `docs/STUDIO.md`.
